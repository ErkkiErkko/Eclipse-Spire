...
import {CardView,HealthBar,PotionIcon,RelicIcon,Rune,Tip} from '@/components/game-ui';
...
export default function Home(){
 ...
 <div className="relic-bar"><div className="relic-icons">{s.relics.map(id=><Tip key={id} title={RELICS[id].name} text={RELICS[id].desc}><button className="relic-token" onClick={()=>open('relics')} aria-label={RELICS[id].name}><RelicIcon id={id} size={36}/></button></Tip>)}</div>{s.relics.length===1&&<span className="starter-relic-label">月之吊坠<small>每场战斗开始时，获得 2 层月辉</small></span>}</div>
